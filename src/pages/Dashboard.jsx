...
function DashboardInner() {
  ...
  useEffect(() => {
    if (searchParams.get('upgraded') === 'true') {
      refetchSubscription()
      showToast('Welcome to Heirloom. Your archive is now unlimited.', 'success')
    }
    ...
    <div className="flex h-screen overflow-hidden bg-surface text-ink">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <div className="hidden lg:flex items-center h-14 px-6 border-b border-outline-variant/60 bg-container-lowest flex-shrink-0">
          ...
          <span className="ml-3 label-meta">An archive in progress</span>
        </div>
          ...
          {treeLoading && (
            <div className="absolute inset-0 flex items-center justify-center bg-surface z-10">
              <div className="flex flex-col items-center gap-3 text-ink-variant">
                ...
                <span className="font-serif italic text-sm">Composing your archive…</span>
              </div>
            </div>
          )}
          {bootstrapError && (
            <div className="absolute inset-0 flex items-center justify-center bg-surface z-10 p-6">
              <div className="bg-container-lowest rounded-md shadow-modal border border-danger-container p-7 max-w-md w-full text-center">
                <div className="w-10 h-10 mx-auto mb-3 rounded-full bg-danger-container text-danger flex items-center justify-center">
                  <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.214 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  </svg>
                </div>
                <h3 className="font-serif text-lg font-semibold text-ink mb-2">Could not open your archive</h3>
